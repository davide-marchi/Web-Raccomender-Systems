...
\begin{itemize}
    \item \textbf{Hit Rate}: Measures how often a relevant item appears in the top-k recommendations for a user.
    \item \textbf{Precision@k}: The proportion of recommended items in the top-k list that are relevant.
    \item \textbf{Mean Average Precision (MAP@k)}: Evaluates ranking quality by averaging precision at each relevant item position.
    \item \textbf{Mean Reciprocal Rank (MRR@k)}: Focuses on the rank of the first relevant item.
    \item \textbf{Coverage}: Measures the diversity of recommended items across all users.
\end{itemize}
